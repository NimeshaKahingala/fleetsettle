...
| **LT-15** | Six features shipped 23–27 Aug (GAP-170/172/173/185/186/188), watched live end to end for the first time | ✅ Closed 28 Aug — five confirmed clean, one new defect found (GAP-191, fixed the same day). Two follow-up items this row left open: CSV row-level content, closed independently by a second session's own pass the same day; GAP-186's manager-role access, narrowed but not fully pinned by that same second pass — see its own row below |
| **LT-16** | Routine-repair narrative walked live to build an end-user guide — non-incident expense, `vehicle_unavailability` create/void, adjustment against a rent due | ✅ Closed 5 Sept — three flows confirmed correct against the database (borne-by resolution, void-and-re-enter preserving the original row, waiver-vs-other `amount_minor` semantics, exact void reversal). One real defect found: **GAP-216**, no odometer on a maintenance expense, which makes GAP-68's service-interval prompt permanently unreachable — planning the fix found a second, independent cause (**GAP-217**, the service-interval save was PATCH-only); both closed together, `fix/gap-216-217-service-reminder`. One candidate finding investigated and rejected as a tool artifact (adjustment `reason`) — see `findings/2026-09-05.md` §5. Not a queue-driven pass; scope came from the guide's own narrative |
...
## LT-15 · Six features shipped 23–27 Aug, none yet watched live end to end

**Opened 28 Aug 2026, and originally numbered LT-14 — corrected the same day.** This file's own summary table already carried a closed LT-14 (20 Aug platform-admin/multi-business regression, row above) when this section was added; the collision wasn't caught until a later pass cross-checked every LT reference in the repo. Renumbered here rather than left standing, the same discipline this row's own account applies to a GAP-number collision found the same day (TRACKER.md §4's note on GAP-196–200). GAP-170, GAP-172, GAP-173, GAP-185, GAP-186 and GAP-188 all closed on `develop`/`main` in the days after the last live pass (23–27 Aug) and each has integration/component test coverage, but none has been exercised by a real browser session against `qa.fleetsettle.com` — the exact gap `run-qa-pass` exists to close (a confirmed-looking write can still connect to nothing, GAP-103/GAP-109's own precedent). The scenario catalogue (`docs/qa/scenarios/`) was refreshed the same day to describe these six as built, ahead of running this row.
