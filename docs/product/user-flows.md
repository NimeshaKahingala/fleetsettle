...
**v1.1.20** — **F-3.4 step 2 states one rent treatment per incident (D3 of the backend accuracy review, 30 Aug 2026).** Re-recording `credit_days`/`extend` while one is already live is refused, not silently re-applied — void the prior one first, where a void path exists (`credit_days` does, via `POST /api/adjustment/{id}/void`; `extend` does not yet). `continue` never blocks a later real choice, since it has no lasting effect to double-apply.

**v1.1.19** — **F-8.3 states a write-off can be partial (D2 of the backend accuracy review, 30 Aug 2026, mirroring `use-cases.md` v1.2.16's UC-90).** The amount need not equal the whole outstanding balance; the remainder stays a genuine receivable until collected or written off separately. Schema in `data-model.md` §10.1/§10.3.

**v1.1.18** — **GAP-188 settled: F-8.1's aggregate half gets a mechanism.** "Reports for the open month show the item flagged as belonging elsewhere" (v1.0's own Accept clause) never said how — `vehicleMonthRowSchema` (UC-70) returns per-vehicle **sums**, so a late fact folded into `earnedMinor`/`costsMinor` had nothing to attach a flag to. **Settled: the late fact becomes its own line** — dated and labelled the same way `IncidentScreen`/`DriverActivitySections` already show one inline (INV-10) — listed beside the sums, never blended into their arithmetic without being separately shown. Scoped to UC-70 alone, matching GAP-188's own filing: UC-73 (the year report) sums across many periods by construction and carries no single "this period" for a fact to be late *against*, so it is out of scope here, not silently widened. F-8.1 and F-9.2 updated. Decided 26 Aug 2026.

**v1.1.17** — **vehicle loans mechanised: new Group F-12, new INV-43 to INV-45, and UC-109 added to F-9.2's report catalogue.** Mechanises `use-cases.md` v1.2.15's **W-68 to W-70** / **UC-106 to UC-109**. The three invariants are the ones that fail silently if missed: **principal repayment never writes an expense and a lender's forgiveness writes no money record at all** (INV-43); a payment and its finance cost are **one transaction** (INV-44); and the parts always add back, with per-payment rounding absorbed by the closing payment rather than carried (INV-45). F-12.2 records explicitly that a loan payment **does not inherit W-35's late-fact flag for free** and must set it. §8's traceability updated. Decided 23 Aug 2026.
...
```
queued ──► suppressed            (a condition failed — at enqueue, dispatch or the final check; always with a reason — INV-12, INV-47)
   ├────► superseded             (a payment reversal started a new round of reminders — INV-49)
   ├────► deferred_verification ──► queued     (verification delivered to this exact number — INV-48)
   │                          └──► expired    (the hold could not be released; surfaces like a failure)
   └────► sending ──► sent ──► delivered ──► read      (outcomes only move forward — INV-50)
             │          └────► failed ──► resend | sent_other_channel | handled_manually
             └────► unknown ──► resend | handled_manually          (never resent automatically — INV-46)
```

**Each `sending` is one attempt** (`data-model.md` §11.1) — its own provider id, its own immutable snapshot of number, template, language, text and configuration, and its own outcome. A resend is a person's decision and makes a new attempt under the same message, back through `sending`; the earlier attempt keeps its record and may still be delivered. A late delivery report for an `unknown` attempt moves it forward to what actually happened. *Redrawn v1.1.22: the previous diagram had no state for a send whose outcome is unknown, so a lost reply could only be read as a failure — and a failure invites exactly the retry that duplicates a message.*
...
**Steps** Name, phone, default per-day rate, default per-trip rate, licence expiry, and how he settles: Daily or Weekly, with the agreed weekday when Weekly. Optionally assign as default driver for a vehicle for a date range.
**Accept** · Rates pre-fill trips and daily cards (U-3) · licence expiry joins the paperwork warnings (F-10.1) · Daily is the default and needs no choice (U-2) · Weekly requires the weekday, which F-4.5 derives his day-fee obligations' due date from (GAP-135).
...
2. **Off-road days and the rent** — enter the dates, then choose *for this incident only*: **rent continues** (default) / **credit the days** pro-rata / **extend the rental** by the lost days. **One treatment per incident (D3, decided 30 Aug 2026)**: once *credit the days* or *extend the rental* has been recorded, re-recording is refused rather than silently applying a second time — void the prior one first (§9.2, `POST /api/adjustment/{id}/void`), where a void path exists. *Extend* has none yet, so a wrong extension needs support's help to correct until one is built. *Rent continues* has no lasting effect, so it never blocks a later real choice.
...
> **Built, 12 Sept 2026 (GAP-135).** `driver.settlement_weekday` (0=Sunday..6=Saturday, required exactly when `settlement_rhythm` is `'weekly'`, `data-model.md` D-5) is the agreed day; the day-fee obligation's `effective_due_on` is the next occurrence of that weekday **on or after** `due_on` — a Friday settler confirmed on a Wednesday is due that Friday, confirmed on the Friday itself is due that same day, not the following one. Scoped to what he owes **us** alone: what we owe **him** (a `driver_fee` obligation, F-5's arrangement-B cost) is our own obligation to pay and stays due the date it was earned, not his settlement day. `SETTLEMENT_RHYTHM_UNSUPPORTED` (17 Aug 2026's interim guard) is retired with it. **Not solved**: a driver who changes rhythm keeps his existing obligations' stored due dates (money is append-only); a day confirmed late uses the rhythm in force at confirm time, not at the business date it names — a plain current-value column has no history to answer "which rhythm applied then" the way `management_fee_agreement`'s own effective-dated shape does. Revisit if that gap is ever felt in practice.
...
· **A vehicle's own arrangement is a default, not a precondition** (GAP-158, corrected 21 Aug 2026) — unlike F-2.1/F-1.7, this flow has no `Pre` on it. A car simply set up for arrangement A but free on the requested dates may take a trip the same as any other; INV-1 alone decides
...
**Accept** · **INV-10** · the record it belongs to shows it inline, dated correctly · reports for the closed month are unchanged; **a report for the open month lists the item as its own line, dated and labelled with the period it actually belongs to** (GAP-188, F-9.2/UC-70) — never folded into an aggregate total without being shown separately.
...
**Steps** Amount, party, reason, date. **The amount need not be the whole outstanding balance** (D2, decided 30 Aug 2026) — a partial write-off is a real business act, and the remainder stays a genuine receivable, chased normally, until it is either collected or written off in an entry of its own.
**Accept** · **INV-14** — never pooled with waivers, never in the same report line · clears the balance from the home screen and receivables **once settled, waived and written-off together reach the full amount** · records the loss against the vehicle it arose from · party history stays intact, and a written-off customer returning next year **arrives with that visible at F-2.1 step 1** · **INV-15** — later payment is a **recovery** linked to the write-off, netting against it, never fresh income.
...
· **UC-70 lists a late fact as its own line** *(added v1.1.18, GAP-188)* — any `obligation`/`trip`/`expense` row posted to the requested period but `belongs_to_period_id`-flagged (F-8.1) elsewhere is returned as a separate, dated, labelled item beside `earnedMinor`/`costsMinor`, which stay exactly the sums they always were — the flag is additive, never a second way of computing the total. Scoped to UC-70 alone; UC-73's year window sums across many periods by construction and has no single "this period" for lateness to be measured against
...
> **CSV ships; PDF stays deferred, re-recorded 12 Sept 2026 on its stronger reason.** ⚑ 17 Aug 2026. The two halves are work of different kinds. **CSV is an ordinary data dump** and is built as specified, W-49 checks included. **PDF's original technical premise no longer settles the question by itself**: `TS §7` (not §8 as this note previously mis-cited — §8 is the environment-bindings table) argued from the Worker's own runtime limits, and now also records an option that renders off-Worker instead. **The deferral survives on the reason that was always the stronger one**: *"neither is needed to satisfy what UC-99 is actually for, which is getting the numbers out."* CSV does that today, and a statement prints from the screen that already shows it — nobody has asked for a document. Deferred until one is genuinely required, not until the runtime allows it; tracked as GAP-136.
...
**Accept** · **INV-13** — append-only at the database level · readable **two ways**: down the log, and from any due, driver or trip (§9.2) · **only failures** surface on the home screen; success is invisible by design · a failed message can be retried, sent by another channel, or marked handled by hand (W-33) · **the record never depended on WhatsApp** — the statement, the slip and the log hold it regardless; an outage costs timeliness, not evidence. · **every attempt is visible, with its own outcome** — a resent message shows both, and the log says plainly when an earlier attempt's outcome is unknown and it might still arrive (W-72) · **`unknown` and `expired` surface on home beside `failed`** — each is a promise the system cannot confirm it kept · delivery reports may arrive twice, out of order, or before the send itself is recorded; the log shows each real change once, in the order it happened, and never moves an outcome backward (INV-50)
...
| **Message uniqueness** | Concurrent enqueues produce exactly one message row — INV-11; concurrent or redelivered queue jobs for one row produce at most one automatic attempt — INV-46. *Reworded v1.1.22: "exactly one send" was more than a unique index can prove* |
...
| A-34 *(added v1.1.22)* | WhatsApp accepts a send, the reply is lost, and the queue redelivers the job | No second automatic attempt; the attempt reads `unknown` and surfaces (INV-46) |
| A-35 *(added v1.1.22)* | The worker disappears between claiming a message and recording the send | The sweep marks the attempt `unknown`; the message never returns to `queued` (INV-46) |
| A-36 *(added v1.1.22)* | A rent reminder queued at 19:59 reaches a worker at 20:01 | Not sent; waits for 08:00 (INV-47) |
| A-37 *(added v1.1.22)* | A lease is started at 23:00 and the post-commit publish fails | The confirmation still goes — by the next sweep — and the window never delayed it (F-10.3) |
| A-38 *(added v1.1.22)* | The verification delivery report arrives before the held lease confirmation's row commits | The confirmation is released by the next sweep, never stranded (W-71) |
