...
**Status:** v1.2.7 — **W-58** added: no record is ever hard-deleted, not only money records — widens W-50's append-only promise from money-bearing tables to every table, including occupancy rows (`vehicle_day_allocation`) and child rows (`payment_allocation`). Decided by the owner, 12 Aug 2026, so that an unexplained figure in production can always be traced to the row that caused it; full mechanism in `data-model.md` §17 (D-11) and `user-flows.md` F-8.5
**Date:** 12 August 2026
