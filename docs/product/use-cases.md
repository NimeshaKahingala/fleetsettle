...
**Status:** v1.2.8 — **W-59** added: names the mechanism behind UC-03's own "own vehicles"/"shared vehicles" matrix cells, silent since they were written — `ownership_share` scopes an owner-manager's capital capability, `management_fee_agreement` scopes a manager's UC-70 visibility, bound to whether the agreement **overlapped the reported period**, not its status today or at period end. Decided closing GAP-1's Wave 3 (tenancy) planning, 13 Aug 2026; full mechanism in `data-model.md` §17 (D-17) and `user-flows.md` INV-34
**Date:** 13 August 2026
