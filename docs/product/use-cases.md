...
**Status:** v1.2.20 — **P14 messaging specified for build, 13 Sept 2026: W-14 revisited on evidence and reaffirmed; W-71 to W-74 added ⚑.** §8 recorded W-14 as a decision to attack, and the 17 Aug deferral waited for real use to attack it with. Real use exists now, and the owner's answers settled it for automatic sending over the Cloud API with business verification deferred — chiefly because delivered and read status is wanted, and no assisted path can produce it. Three mechanics Group I required without saying how to keep become decisions: **W-71**, verification before money is a *hold* released by an amount-free message to that exact number, never a drop; **W-72**, an attempt that may have reached the recipient is never resent automatically; **W-73**, a payment reversal re-arms reminders as a new round and stops the old one; **W-74**, each business sends from its own WhatsApp account and number, in its own name — never a shared FleetSettle number. The owner also set the verification hold at 3 days and chose one WhatsApp message per handover photo. Answers on the template wording add **UC-110** — a driver gets a receipt when he pays — and remove the odometer-photo request from the rent reminder (UC-81, W-18, UC-14). The prerequisites table is corrected against Meta's own documentation — the 250 limit counts unique recipients across the portfolio, unverified accounts hold 250 templates, and the template count was never twelve. Mechanics: `user-flows.md` v1.1.22; schema: `data-model.md` v1.1.21 §11.1; reasoning and sequencing: `docs/evaluations/P14-MESSAGING-PLAN-2026-09-13.md`. No golden-fixture figure moves.

**v1.2.19** — **UC-04 catches up to the settlement-weekday input `user-flows.md` F-1.6 already documented (GAP-135, 12 Sept 2026).** F-1.6 gained "how he settles: Daily or Weekly, with the agreed weekday when Weekly" when the derivation was built; this use case, F-1.6's own source, still listed only name, phone, fees and licence expiry — the pair disagreeing about what UC-04 covers (PR #186 review, PR #187). No new rule: the mechanics stay owned by F-1.6, this only names the input at the use-case level.
...
| **W-18** | Odometer readings arrive by **mixed routes** — a photo, read in person, or only at return. *(⚑ 13 Sept 2026: the photo is no longer requested by the WhatsApp rent reminder — UC-81.)* | The reading records *how* it was obtained, since a photo is verifiable and a spoken figure is not. Monthly excess is provisional; the return reading reconciles it. UC-14 |
...
| **W-71** ⚑ *(added v1.2.20)* | **Verification before money is a hold, never a drop.** An amount-free message proves the number; a money message for a recipient not yet verified *on that exact number* waits, keeps its place, and is released into the ordinary send checks when that message is delivered. A hold that cannot be released expires with a reason and surfaces like a failure. | Suppressing the held message outright would spend its one-send-per-trigger slot, so the lease confirmation that exists to record agreed terms would never go at all. Binding verification to the number rather than the person stops a late delivery report for an old number vouching for a new one. §6.10, UC-86 |
| **W-72** ⚑ *(added v1.2.20)* | **A message whose earlier attempt may have reached the recipient is never resent automatically.** An unknown outcome is shown, not guessed at; resending is a person's decision, logged, and it may produce a second delivery. | Queues and networks deliver at least once, so a reply lost after WhatsApp accepted a message looks exactly like a failure. Retrying automatically turns every such blip into a duplicate message about money. The system can promise no *automatic* duplicate; once a person asks for a resend it cannot promise exactly one delivery, and says so. UC-87 |
| **W-73** ⚑ *(added v1.2.20)* | **Reversing a payment re-arms its reminders as a new round and stops the old round in the same step.** If a receipt already went out, the correction names that message, and a correction message goes out straight away. | Without a new round, the re-armed reminder collides with the one already sent and is silently skipped; without stopping the old round, both rounds see an unpaid due and both send. Waiting for "the next message" could leave a written receipt for money that never arrived standing for a month. UC-93 |
| **W-74** ⚑ *(added v1.2.20)* | **Each business sends from its own WhatsApp account and number, in its own name.** FleetSettle never sends one business's messages from a shared number or under its own name. A business with no connected account sends nothing; the platform admin connects one by hand, until enough businesses make self-service connection worth building. | A customer told about money by a sender he does not recognise does not trust the message, and Meta requires the sender's name to represent the business sending. A shared number would also pool what must stay separate: one business's blocked messages would slow every business's sending, and a reply could not say which company to call. An unverified Meta account holds only two numbers, so stacking every business inside one account stops at the second. UC-86, W-49 |
...
| Route | Note |
|---|---|
| Photo, sent or shown to you directly — ⚑ **not** requested by the WhatsApp rent reminder (UC-81, decided 13 Sept 2026) | Verifiable. It now needs its own contact, because replies to the reminder are not read (W-45). *Until 13 Sept this row read "requested with the rent reminder … he is already being messaged about money that day"* |
...
*Counts:* everything recognised in the accounting period (W-40) — rent for periods falling in it, daily lease amounts by day, trips by closing date (W-41), a customer's own contribution to an incident (GAP-175), and an insurer settlement once actually received (W-11). **Net, not gross (D1, 31 Aug 2026): a waiver reduces what is counted as earned** — a waiver is a discount decision on what was ever really collectible (FL §5.73's "the month shows the 340 charged and the 340 waived" — both facts stay visible in UC-99's own export, this report only ever showed the net total). Costs gain a write-off (UC-90) the same way — a loss you were handed, booked in the month it was accepted.
...
Configured per rental — default three days before, again on the due day, and once if it goes overdue. ⚑ **The reminder does not ask for the odometer photo — decided by the owner, 13 Sept 2026.** WhatsApp replies are not read (W-45), so a photo sent in reply would reach nobody, and a message about money must not ask for something no one will receive. The reading is collected separately (UC-14). *Until then this line said the reminder was where you ask for the photo, "because he is already being contacted about money that day" — true, but only useful if there is somewhere for the photo to go.*
...
Optional, on recording a payment. Amount, period covered, balance if any. ⚑ **And any credit held for him when he has paid more than was owed** — decided by the owner, 13 Sept 2026: a receipt saying nothing is owed while money is held for him is incomplete, in writing.

**UC-83 Customer gets the closing figures**
On closing a lease (UC-16). Final period charged, any excess mileage, deposit refunded, applied **or held — with the amount and the date the hold ends** — and the closing balance either way. ⚑ *When a held deposit is released, a second message says what was refunded and anything kept, with the reason — decided by the owner, 13 Sept 2026, because a deposit is exactly the argument this message exists to prevent.* The message that ends the relationship cleanly — and the one you will be glad exists if he later remembers the deposit differently.
...
**UC-110 Driver gets a receipt when he pays** ⚑ *(added v1.2.20, 13 Sept 2026)*
Triggered when you record a payment **from** a driver — most often his daily lease amount. Amount, date, what it covered, his two balances, never netted (W-2), and any credit held against his coming days when he has paid ahead.
*Why it exists:* UC-82 gives a customer a receipt and UC-84 tells a driver when *he* is paid, but a driver handing over cash each evening got nothing in writing — the direction of money arrangement B runs on every day. It settles "I paid you on Tuesday" the same evening. Added by the owner while the template wording was being drafted; numbered after the last use case rather than squeezed into Group I's sequence, so no existing reference moves.
...
- **Verification before money** — a number is verified by a first successful delivery before any amount is ever sent to it. Amounts do not go to unconfirmed numbers. **That first delivery is its own amount-free message, and a money message waiting on it is held rather than dropped (W-71)** — verification belongs to the exact number, so changing someone's number starts it again.
...
*Before any of this applies, the business's own WhatsApp account has to be connected (W-74).* The business creates it in its own name and shares it with FleetSettle; the platform admin connects it. Until then that business sends nothing, and there is nothing to configure.
...
**One send per trigger, enforced.** A retry, a restart, or two overlapping schedules must never produce two messages for the same due at the same stage. Duplicated messages about money erode trust faster than silence does. **What "once" can honestly promise (W-72):** no message is ever sent twice *automatically*. When an attempt's outcome is unknown the log says so, and a person decides whether to send again — accepting that the first may still arrive.
...
| Step | Notes |
|---|---|
| Meta Business Manager account and **business verification** | Required to raise sending limits and template capacity, not to send. **Deferred, 13 Sept 2026.** An unverified portfolio can message **250 unique recipient numbers per rolling 24 hours outside a customer-service window, shared across the whole portfolio**, and hold **250 templates per WhatsApp Business Account, each language variant counting separately** — both far above this business. Delivery and read reports do not depend on verification. *Corrected against Meta's own documentation: this row said "roughly 250 business-initiated conversations", which counts the wrong thing* **For the platform:** acting on a business account FleetSettle does not itself own needs Meta's app review for advanced access — required before a second, unrelated business is connected, not before the first |
...
| A **clean phone number** | Must receive an SMS or voice OTP, must not already be active on WhatsApp, and VoIP numbers are generally rejected. **One per business (W-74)** — each rental business brings its own, held by someone in that business, since Meta's setup codes arrive by SMS |
| **Template approval, per message per language** | Approval typically ranges from minutes to about two days each. Record the approved template names and language codes — a variable-count mismatch is a common cause of send failures. **Languages: English and Sinhala (`si_LK`).** **The count is not twelve**, the figure carried elsewhere until 13 Sept 2026: fixed template text needs a template for each distinct situation. **The inventory and its count live in `docs/evaluations/P14-MESSAGE-TEMPLATES-DRAFT-2026-09-13.md`, the authority for them** — named there rather than counted here, so this row cannot go stale again |
...
| Cost | Charged per message or conversation, varying by country and category, plus any provider fees. Utility volumes here are small: a handful of reminders and confirmations per rental per month. **Sri Lanka moves to its own, lower utility rate card on 1 October 2026** (Meta pricing, checked 13 Sept 2026) |
...
*What it must undo, all of it:* the due goes back to unpaid or partly paid, the arrears reappear, the customer or driver balance returns to what it was, and the reminder re-arms so he is chased again — the stateful rule (§6.10) running in reverse, which is the direction nobody remembers to build. ⚑ *And it undoes credit before dues:* if the payment left credit, the correction uses that credit up first, and only what remains reopens a due or is absorbed — corrected to 49,000 from a 50,000 payment that settled a 45,000 due, he owes nothing and keeps 4,000 credit (decided 13 Sept 2026; today's code gets this wrong, GAP-229).

*And one thing it cannot undo.* If a receipt was already sent (UC-82 to a customer, UC-110 to a driver), he has been told in writing that you received money you did not. That message stays in the log because the log is append-only, so the reversal records which message is owed a correction, and a correction message says so plainly, straight away (W-73) — waiting for the next message could leave a written receipt standing uncorrected for a month. The reminders come back as a new round tied to this correction, and the old round stops in the same step, so he is chased once, not twice. Silently reversing a payment you have already confirmed in writing is how a small banking failure becomes a dispute about your honesty.
...
*Delivery, decided 17 Aug 2026, re-recorded 12 Sept 2026 — the spreadsheet ships; the document stays deferred, on a different reason than the one that first justified it.* ⚑ The two halves of this use case turned out to be work of completely different kinds. **The spreadsheet is an ordinary data dump** and is built as specified. **The document's original technical objection no longer holds by itself**: this document argued from the Worker's own runtime limits (`tech-stack.md` §7) — true then, still true, and no longer the whole picture, since `TS §7` now also records an option that renders off-Worker. **The deferral survives on its other, always-stronger reason**: **neither is needed to get the numbers out**, which is what this use case is for — the spreadsheet does that today, and a statement can be printed from the screen that already shows it. Deferred until there is a real, stated need for a document rather than a page, not until the runtime allows one — tracked as GAP-136; mechanics in `user-flows.md` F-9.3.
...
### Provenance of W-71 to W-74, and what the P14 plan declined

These four came from sizing P14 for build (`docs/evaluations/P14-MESSAGING-PLAN-2026-09-13.md`), after two independent validation passes against the code and against Meta's and Cloudflare's own documentation — recorded here for the same reason as W-27 to W-36 and W-63 to W-67. W-71 to W-73 make explicit something Group I already required but had not said how to keep. W-74 settles something Group I never addressed at all, because it was written for one business — and it was the owner, not either validation pass, who asked the question. None changes what a customer or driver is told, except that a receipt correction now arrives at once and the sender is always his own rental company.

| Declined | Why |
|---|---|
| Assisted, tap-to-send messaging | No delivery or read status, and a number can never be "proven by a first delivery" if nothing reports delivery |
| Automating only the rent reminder and settlement summary | Halves the approvals, but the owner wants delivery status on every message, which only the automatic path gives |
| Retrying an attempt automatically after a timeout | A timeout cannot tell a lost message from a lost reply; retrying duplicates the second case (W-72) |
| Suppressing a money message outright when its number is unverified | Spends its one-send slot, so it could never go (W-71) |
| Verifying the person rather than the number | A late report for an old number would vouch for a new one (W-71) |
| Leaving the receipt correction to "the next message" | The next message may be a month away (W-73) |
| Condition photos as one combined image or document | Needs a renderer this runtime does not have (UC-99's PDF deferral); one template per photo costs a few cents and keeps each photo's own delivery record |
| One FleetSettle number for every business | Customers would not recognise the sender, Meta requires the name to represent the sending business, and one business's blocked messages would slow all of them (W-74) |
| Self-service WhatsApp connection inside FleetSettle, built now | Right once there are many businesses; with one live business it is a build and a Meta app review for a problem that does not exist yet (W-74) |
...
- **W-14, fully automatic messaging.** At this volume — a handful of rentals and one bus — automation buys little and brings Business API onboarding, template approval, per-message cost, and a new class of silent failure to monitor. Assisted sending would have delivered nearly the same value at nearly no cost. Worth revisiting if the fleet grows rather than before.
  - *Revisited 12–13 Sept 2026, on the evidence this entry asked for, and reaffirmed.* Real use began; the owner can dedicate a clean number, expects most customers and drivers to opt in, and wants delivered and read status — which assisted sending cannot produce, since a message handed to someone's own WhatsApp reports nothing back. Two cheaper shapes were considered and declined: assisted tap-to-send (no delivery status, and §6.10's verification rule becomes unimplementable) and automating only the two unattended reminders (it saves approvals but not the delivery-status requirement). **The entry stays in this list**: automation's new class of silent failure is real, and W-72 is the part of it that could not be designed away.
...
**Started 13 September 2026, by owner decision, now that real use exists** (§8, W-14). Template approval returns to a critical path, but only for the transport itself: the triggers, holds, log and screens are built and tested first against a stand-in transport, which is what W-21 always promised.
