...
| **Template approval, per message per language** | Approval typically ranges from minutes to about two days each. Record the approved template names and language codes — a variable-count mismatch is a common cause of send failures. **Languages: English and Sinhala (`si_LK`).** **The count is not twelve**, the figure carried elsewhere until 13 Sept 2026: the six messages above, plus the amount-free verification message W-71 needs, the receipt correction W-73 needs, and a condition-photo template for UC-80, since a template carries a single image. The final inventory is fixed when the wording is written |
