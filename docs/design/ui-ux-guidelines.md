...
**Status:** v1.2.5 — M-28: `PhotoCapture`'s upload reverses §6.3's own prior wording, from a presigned R2 PUT to the Worker's `R2` binding — A7/GAP-16's plan, decision 1
**Date:** 9 August 2026
...
**Back must always mean back.** A sheet dismisses; a route pops. Sheets never push history entries on desktop but do register a history entry on mobile so the hardware back button closes them — this is the single most-missed Android behaviour in web apps.
...
| Component | Spec |
|---|---|
| `AppShell` | Safe areas, tab bar, offline banner, toast host. Owns the single scroll region |
| `Screen` | App bar + scroll region + optional sticky action. Enforces `scroll-padding-bottom` |
| `Sheet` | Bottom sheet. Drag-to-dismiss **plus** a visible close button (M-23). Focus trapped, `aria-modal`, restores focus, registers a history entry on mobile |
...
| Upload | **M-28.** Through the Worker's `R2` binding, one request per photo, per-photo progress (`"uploading"`/`"uploaded"`/`"error"`, not a byte-level percentage) and retry — **not** the mutation queue, which is for JSON writes, and **not** a presigned PUT: see M-28 below |
