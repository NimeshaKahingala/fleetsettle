...
**Status:** v1.10 — **M-39: the primary action is no longer `position: sticky`.** GAP-171 (25 Aug) fixed a *plain tap* landing on the CTA instead of a form's own last field by reserving real `padding-bottom` around the scroll region's content, sized to the CTA's footprint. A live pass on 28 Aug found the same mechanism still swallows taps in a different shape it doesn't cover: a **short** list (`Show all (N)`) sitting at rest under the CTA's own band, confirmed with `elementFromPoint` hit-testing rather than assumed from a bounding-box measurement. The root cause both share is the CTA sharing the scroll region's own coordinate system at all — a sticky element paints over whatever normal-flow content occupies its band at the *current* scroll position, and no amount of padding tuning closes that for every possible content shape. §4.2's frame changes from "scroll region with a sticky child, reserving scroll-padding-bottom" to "scroll region and primary action as two ordinary flex siblings" — the primary action's row is sized by its own content (`shrink-0`) and the scroll region simply fills whatever height is left (`flex-1`), so scrolled content can never reach the action's band to begin with. `scroll-padding-bottom` and the reserved `padding-bottom` both become unnecessary, not tuned further. Filed and fixed the same day (GAP-199).
**v1.9** — **M-37 and M-38 added, §3.2 gains item 8, and §3.3 fixes the vehicle-loan vocabulary.** M-36a left three bell rows count-only *"rather than inventing a new destination"*; M-37 supplies them, and gives each a **different** answer because checking per row found the three are in genuinely different states — one destination already exists, one is already on the page, and only one needs building. §3.2 gains **loan instalments due** so the bell counts them (UC-107, F-12.4). M-38/§3.3 fix the loan words: `finance` maps to **"Interest and charges"** at the edge, and none of the banned column is caught by `check-forbidden.mjs`, which is precisely why it is written down. Serves `use-cases.md` v1.2.15 / `user-flows.md` v1.1.17. Decided 23 Aug 2026.

**v1.8** — **§5.1's colour table updated to Tactile Ops Phase 1** (`TACTILE-OPS-REDESIGN-2026-08-21.md` §5D) — full cool-neutral/navy palette replacing House Style's warm/terracotta pair, built and passing `guard`/`lint:css`/`typecheck`/tests. Not yet a numbered `M-35` decision — that lands once Phase 8 closes the whole 9-phase redesign; this bump only keeps the token table from lying about what `tokens.css` ships today. Updated 22 Aug 2026.
...
| **M-26** ⚑ | **Landscape is supported, not locked.** Below `md` in landscape the app bar collapses to 44px, the tab bar becomes icon-only at 44px, and the primary action row stays put | Locking orientation is the obvious answer and it is not available: **WCAG 2.1 SC 1.3.4 Orientation** forbids restricting content to one orientation unless it is essential, and a ledger is not essential in that sense. The manifest's `orientation` field is also ignored outside standalone mode and on iOS, so a lock would be a lock in name only. That leaves making landscape work: ~192px of content between the chrome is enough for the day card once the chrome gives back 24px |
...
| **M-37** ⚑ *(added 23 Aug 2026)* | **Three of §3.2's bell rows gain a destination, and each gets a different answer rather than one pattern imposed on all three.** M-36a left them count-only, deliberately, *"rather than inventing a new destination"* — this supplies the destinations now that the question has been asked properly of each. **Rent due → the receivables report**, which already exists (`ReceivablesReportScreen`); a wire-up, no new screen. **Earlier days → close the sheet and scroll to the Home section already rendering exactly that content** — the unconfirmed-day groups are on the page, so a new route would duplicate them. **Deposits to release → one new minimal list screen**, each row opening that party's own detail where the deposit already lives; this is the only one of the three with nowhere at all to go today. §3.2 also gains **item 8, loan instalments due**, so the bell counts it (UC-107). Every new tap target meets M-1: 44 × 44 CSS px, ≥ 8px apart — the sheet's rows are `py-3` today and do not, so they change. | M-36a's count-only decision was right for its own pass and is not being reversed as an error; what it lacked was the per-row check, and doing that check found the three rows are in genuinely different states rather than one. Two alternatives were declined. **Scroll-to-section for all three** would have put a deposits section on Home that nothing asked for, and would bypass the receivables screen that already exists and is the better destination. **A new screen for each** would duplicate the unconfirmed-day content already on Home — precisely what M-36a's own "rather than inventing a new destination" was cautioning against. Read/unread on the bell stays **declined** (M-36a): rows are derived live, so dismissing one would hide unpaid money behind a badge that then reads wrong, which is the W-56 failure applied to the one widget people trust to say what they missed. |
| **M-38** ⚑ *(added 23 Aug 2026)* | **Vehicle loans get a fixed interface vocabulary, decided once here rather than invented per screen.** `finance` is a database category value and is mapped to **"Interest and charges"** at the edge like every other category label — GAP-158/159 were exactly this shape, a raw enum reaching the interface unmapped, so the mapping is a requirement rather than a nicety. | U-6 bans accounting vocabulary outright and `scripts/check-forbidden.mjs` enforces a hard list at write time (*accrual, accrued, receivable, payables account, current account, allocation, debtor, creditor, reconciliation*). **None of the loan words hit that regex** — which is exactly why they are fixed here: the guard would not have caught "amortisation" or "outstanding principal", and a bus owner does not use either. |
...
7. **Trips in progress**
8. **Loan instalments due and overdue** *(added v1.9 — UC-107, F-12.4)*, derived on read against the loan's own schedule, never from a stored counter or a scheduled job
...
### 3.3 Vehicle loan vocabulary *(added v1.9, M-38)*

Fixed here so it is not invented per screen. None of the "never says" column is caught by `check-forbidden.mjs` — that guard covers accounting jargon, and these are lender's jargon, which a bus owner uses no more readily.

| Concept | The interface says | Never says |
|---|---|---|
| Remaining balance | **Remaining to pay** | outstanding principal, balance outstanding |
| The finance portion of a payment | **Interest and charges** | finance cost, interest expense |
| The principal portion | *not shown at all* | principal, capital portion |
| `amortisation_method` | *never shown* | amortisation, flat rate, reducing balance |
| Being behind on instalments | **Behind by Rs X** | arrears, overdue balance |
| Closing a loan early | **Settle and close** | early settlement, foreclosure |
| Principal the lender forgave | **Waived by the lender** | debt forgiveness, write-back |
| `expense.category = 'finance'` | **Interest and charges** | finance, the raw value |

**"Remaining to pay" is the figure on the lender's letter**, which is why it is the one shown: it is the number the owner can check against a document. Principal outstanding is derived internally and never surfaces — showing both would invite the question of why they differ, whose honest answer is a lesson in amortisation nobody asked for.
...
├─────────────────────────────┤
│                             │
│ Scroll region (flex-1)      │  ← the only scrolling element;
│   gutter 16px               │     fills whatever height the
│                             │     action row below doesn't take
├─────────────────────────────┤
│ Primary action row (M-24)   │  56px, full width minus gutters —
│ (shrink-0)                  │  a flex sibling, not a child inside
...
- The keyboard is an overlay and **`dvh` does not account for it by specification**. The bottom action row uses `bottom: env(keyboard-inset-height, 0px)` where supported, with a `visualViewport` resize listener as the fallback ([virtual keyboard handling](https://www.bram.us/2021/09/13/prevent-items-from-being-hidden-underneath-the-virtual-keyboard-by-means-of-the-virtualkeyboard-api/)).
- **M-39 (28 Aug 2026): the primary action is a flex sibling below the scroll region, never `position: sticky` inside it.** A sticky element shares the scroll region's own coordinate system, so it paints over whatever normal-flow content occupies its band at the *current* scroll position — including scroll-top, on any screen whose content starts there. That is what GAP-171 found for a form's last field and what a later pass (GAP-199) found again for a short list sitting at rest under the action: the same root cause, two different content shapes. As a flex sibling, the scroll region's own height is already exactly "whatever the action row doesn't take" — scrolled content can never reach the action's band, so there is nothing left to overlap and nothing to reserve. `scroll-padding-bottom` and a matching `padding-bottom` on the scroll region's content are both unnecessary, not tuned further; WCAG 2.4.11 Focus Not Obscured is satisfied by construction rather than by a padding calculation that has to be right for every content shape.
...
| `Screen` | App bar + scroll region + optional primary-action row. **M-39 (28 Aug 2026):** the action row is a flex sibling below the scroll region, never `position: sticky` inside it — see §4.2. **Added 21 Aug 2026:** `HomeScreen` uses its one `action` slot for the needs-attention bell (M-36a) — every other screen's use of that slot is unaffected |
...
*The mobile-specific part.* Focusing the first error must scroll it clear of the message that is about to appear beneath it, so the scroll target is `field height + 20px message`. **M-39 (28 Aug 2026): no longer `+ scroll-padding-bottom`** — the action row is a flex sibling below the scroll region (§4.2), not a sticky overlay inside it, so a focused field can never land behind it regardless of scroll position; there is nothing left in this calculation to reserve for. One line per error on mobile; wrapping is allowed from `md` up.
...
| **Vehicles** | `/vehicles`, `/vehicles/:id`, `/vehicles/:id/calendar`, `/vehicles/:id/lease/new`, `/vehicles/:id/daily-lease/new`, `/leases/:id`, `/leases/:id/close` | Vehicle rows use registration-first hierarchy, vehicle/status icons, arrangement/status badges, clear next actions, and calendar cells keep glyph + colour. Create/close routes keep one primary action row and level-2 disclosure hierarchy. |
...
| **2.4.11 Focus Not Obscured (AA, new in 2.2)** | The bottom action row could hide the focused field if it shared the scroll region's own coordinate system. Satisfied by construction (M-39, §4.2) — the action row is a flex sibling below the scroll region, not a sticky child inside it, so scrolled content can never reach its band |
...
| Focused field never intersects the primary action row | SC 2.4.11 |
