...
| **D-5** | `obligation.effective_due_on` maintenance | **Guarded at the write path, 17 Aug 2026 — the divergence is now unreachable rather than merely undetected.** `confirmDay` refuses any day whose driver is not `'daily'` (`SETTLEMENT_RHYTHM_UNSUPPORTED`, 409) instead of writing an `effective_due_on` it cannot derive, so the "silently wrong for a `'weekly'` driver" case below can no longer be reached without an explicit refusal first. **Deliberately not a `CHECK` pinning the column to `'daily'`**: UC-31, UC-37 and FL F-4.5 all describe weekly settlement as a real arrangement of this business, so constraining it away would make the schema contradict the product rather than admit an unbuilt feature. The rhythm stays licensed; only the unbuilt derivation is blocked, and **GAP-135 remains open as the feature it always was** — the guard buys correctness, not completeness. Original finding follows. **Confirmed unimplemented, 16 Aug 2026 — filed as GAP-135.** This row's own original text ("derived from `driver.settlement_rhythm` at creation... worth confirming") describes intent that was never built: every current write path, including `confirmDay.ts`'s driver-side `driver_fee` obligation, sets `effective_due_on = due_on` unconditionally; `driver.settlement_rhythm` (defaults `'daily'`) is written by no endpoint and read nowhere. Harmless for a `'daily'` driver, but silently wrong for the `'weekly'` case this column exists to describe — a driver who settles every Friday reads as overdue on Thursday, the exact scenario D-5 was written to prevent |
