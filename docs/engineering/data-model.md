...
CREATE TABLE driver (
  ...
  settlement_weekday    smallint,             -- GAP-135/D-5: 0=Sunday..6=Saturday,
                                               -- the same convention as
                                               -- daily_lease_pattern.pattern_weekdays
                                               -- below; required exactly when
                                               -- settlement_rhythm is 'weekly'
  created_at            timestamptz NOT NULL DEFAULT now(),
  CHECK ((settlement_rhythm = 'daily' AND settlement_weekday IS NULL)
...
CREATE TABLE lease (
  ...
  closure_summary_shown_at timestamptz,         -- ⚑ never written (§13, corrected 19 Aug 2026) — column kept, not a live gate
  closed_at         timestamptz,
  created_at        timestamptz NOT NULL DEFAULT now()
);
...
**GAP-187, closed 12 Sept 2026.** This table names a party but carries no `posted_period_id` of its own, so migration 0031's archive-guard view (§13's companion, `archive_guarded_party_column`) structurally cannot see it the way it sees `obligation`/`deposit`/`advance`/`payment` — no trigger attaches here. A *committed* correction is still safe regardless: it materialises into exactly those trigger-guarded tables, so an archived party is refused there. What was missing was feedback on an ordinary *draft* save, which touches only this table and could name an archived party with nothing refusing it until the eventual commit, arbitrarily later. Closed at the application layer rather than by widening the trigger view — `saveOpeningBalance` (`domain/opening-balance.ts`) takes a `FOR SHARE` lock on each named driver/customer row before writing anything, the same lock strength (and the same "don't block ordinary readers of each other, only the archiver" reasoning) migration 0037's own triggers already use.
...
**INV-18 (deposit not settled before the closure summary is shown)** is the one deliberately left to the application: it is a workflow-ordering rule, not a data rule, and a trigger would enforce it against imports and fixtures too.

**⚑ Corrected 19 Aug 2026 — the mechanism above was never built the way this paragraph described.** `lease.closure_summary_shown_at` is never written by any code path (grepped every write to the `lease` table, api/src — confirmed while fixing GAP-141/F-4, a live QA finding). `api/src/domain/lease-closure.ts`'s own comment on `getLeaseClosureSummary` states the actual, deliberate mechanism plainly: "INV-18 itself is enforced by this endpoint simply existing to be called (and being called) before `settleLeaseDeposit`, not by a stored flag." That is real reasoning, not an oversight left unrecorded — it just never made it back into this document, so the two disagreed for however long P5+ has been built. **The column stays** (dropping it is a separate decision nobody has asked for, and a future session may yet want the stronger, server-enforced version this paragraph originally specified) — but it is unused today, and `settleLeaseDeposit` has no check against it. The practical consequence: the five-step wizard's own ordering is what protects a real user, but nothing stops a direct API call from settling a deposit that was never shown its own closure summary first.
...
| 18 outstandings before deposit | Call ordering — `getLeaseClosureSummary` (step 4) is the wizard's own step before `settleLeaseDeposit` (step 6); ⚑ not a stored-flag gate, corrected 19 Aug 2026, §13 | **App** (§13), **not server-enforced** |
...
| **D-5** | `obligation.effective_due_on` maintenance | **Built, 12 Sept 2026 — migration `0040` adds `driver.settlement_weekday` (0..6, paired with `settlement_rhythm` by a `CHECK`: `'daily'` requires NULL, `'weekly'` requires a real weekday), and `confirmDay`'s `daily_amount` obligation derives `effective_due_on` as the next occurrence of that weekday on or after `due_on` (`nextOccurrenceOfWeekday`, `packages/shared/src/dates.ts`) — "on or after", not strictly after, so a day confirmed on the settlement day itself is due that same day. `SETTLEMENT_RHYTHM_UNSUPPORTED` is retired with it: the guard bought correctness while the derivation didn't exist, and keeping a dead refusal path after it does would only mislead the next reader. **Scoped to `owed_to_us` alone** — what the business owes the driver (`driver_fee`, `trip.ts`) is its own obligation to pay, not his rhythm to settle, and stays due the date it was earned. **Opening-balance arrears are deliberately excluded**: `originalDueDate` there is an explicitly supplied historical deadline (UC-09's "real original due dates"), not a day this ledger scheduled, so applying today's rhythm to it would silently overwrite a stated fact with a computed one — `effective_due_on = due_on` unconditionally for that path. **No effective-dating on the weekday itself**: a driver who changes rhythm keeps his existing obligations' stored `effective_due_on` unchanged (money is append-only); only obligations written after the change use it. A late-confirmed historical day uses the rhythm in force *at confirm time*, not at the business date it names — recorded as a known limitation rather than solved, since a plain current-value column has no history to derive an as-of answer from (unlike `management_fee_agreement`'s own effective-dated shape); revisit if a driver's rhythm changing while backlogged days are unconfirmed turns out to matter in practice. **Verified against both live environments before the migration, not assumed from "no endpoint writes this"**: production's `driver` table held zero rows, QA's six were all `'daily'` — no backfill needed. Settable at driver creation only (`createDriverRequestSchema`); no driver-edit endpoint exists yet for any field. Closes GAP-135 |
