...
/**
 * F-2.4/UC-15/W-17, one transaction: `adjustment` plus the obligation it
 * touches. A waiver (or `auto_waiver`) raises `waived_minor` only — the
 * original `amount_minor` stays "the 340 charged" (DM §10.3's own example),
 * so the month can show both the charge and the waiver. Every other type
 * is a real change to what is owed and adjusts `amount_minor` by
 * `sign * amountMinor` directly.
 */
