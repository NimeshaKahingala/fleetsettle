...
export async function replaceExpense(
  ...
  if (existing.voidedAt !== null) throw new ExpenseAlreadyVoidedError();
  await assertExpenseNotFinanceLinked(writer, input.businessId, input.expenseId);
  // GAP-226 (Copilot review, PR #182): the schema can't reject `category:
  // "finance"` outright the way createExpenseRequestSchema does — a replace
  // of an *already* finance-linked row legitimately echoes that category
  // back, and the check above already 409s that case before this line
  // runs. What must still be refused is turning an *unlinked* expense into
  // one — the same create-time hole (packages/shared/src/schemas/expense.ts),
  // reachable here because assertExpenseNotFinanceLinked only inspects the
  // *existing* row's linkage, never the incoming payload's category.
  if (input.category === "finance") {
    throw new ValidationError(
      "finance is generated by the loan-payment flow, never chosen directly",
    );
  }
