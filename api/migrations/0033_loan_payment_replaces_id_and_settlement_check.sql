...
-- balance), not to drop the floor on settlements altogether. Widened rather
-- than dropped-and-reinstated as a bare `>= 0`, so the ordinary-payment half
-- (amount_minor > 0) is untouched and 2847794's full-forgiveness case
-- (settlement = 0) still passes.
--
