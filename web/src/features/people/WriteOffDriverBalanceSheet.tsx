...
export interface WriteOffDriverBalanceSheetProps {
  open: boolean;
...
/** F-8.3/UC-90: standalone driver loss, not tied to one visible obligation. */
export function WriteOffDriverBalanceSheet({
  open,
  onOpenChange,
  driverId,
  today,
}: WriteOffDriverBalanceSheetProps) {
  const api = useApi();
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange} title="Write off balance">
      ...
      >
        ...
        <Field label="Reason" htmlFor="writeOffDriverReason">
          <Input
            id="writeOffDriverReason"
            value={reason}
