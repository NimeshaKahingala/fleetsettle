...
export const partnerPayoutResponseSchema = z.object({
  ...
  userId: z.string().uuid(),
  amountMinor: z.string(),
  // GAP-185/UC-107: 'loan_on_behalf' is generated by domain/vehicle-loan.ts
  // alone, never accepted on recordPartnerPayoutRequestSchema — a payment
  // against a named owner's own loan liability, not a manual entry.
  kind: z.enum(["payout", "partner_settlement", "loan_on_behalf"]),
  occurredOn: z.string(),
  // GAP-60/D-16/F-8.6: "what corrected this?", answered from the record.
  replacesId: z.string().uuid().nullable(),
});
...
export const listPartnerPayoutsQuerySchema = z.object({
  userId: uuidSchema.optional(),
  kind: z.enum(["payout", "partner_settlement", "loan_on_behalf"]).optional(),
});
