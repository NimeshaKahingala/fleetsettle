...
1. **Live-and-silently-wrong outranks unbuilt.** GAP-3, GAP-56, GAP-75 and now GAP-90 each jumped the queue on it. A missing screen is visible; a wrong number is not.
...
| | Order | Note |
|---|---|---|
| **Track A** | ~~A9a~~ → ~~A6~~ → ~~GAP-74~~ → ~~GAP-41~~ → ~~GAP-72~~ → ~~F1~~ → **F2 (server half) → A17 → A10a → A10b → A13 → A15 → A16** → A8 → A14 → A7 → A9b | A9a done 5 Aug, A6 done 6 Aug, **the three report-query gaps and F1 done 8 Aug** — F1 closed GAP-84 and found one sibling (GAP-87, unfixed, correctly unscheduled). **F2 and A17 are new, 9 Aug** — F2's server half is the revoked-member 500 (GAP-90), A17 is two route-def validation holes (GAP-92, GAP-93). **A10 splits in two** — A10a builds, A10b needs a `doc-change` first. A8/A14/A7/A9b are deliberately deferred behind the client work this pass prioritises |
| **Track B** | ~~B0~~ → ~~B11~~ → ~~B10~~ → ~~B14~~ → ~~B0b~~ → ~~B12~~ → ~~B13~~ → ~~B3 (core)~~ → ~~GAP-81~~ → ~~B4 Wave 1~~ → ~~12a `doc-change`~~ → ~~GAP-85~~ → **B4 Wave 2 → F2 (client half) → B9 → B15 → B6** → B5 → B2 → B7 | **B14, B0b, B12, B13, B3-core, GAP-81, B4 Wave 1, 12a's `doc-change` and GAP-85 all done.** **B4 Wave 2 is the open half of B4** — not blocked, its `doc-change` landed 9 Aug. **F2's client half** (the `Loading…`-forever branch) should ship with its server half rather than separately, since either alone leaves the endpoint wrong in a different way. **B5 and B2 stay behind them, deliberately and at a cost** — see "What this re-sequence gives up". **V1 is not code**, does not block any of this, and can run in parallel whenever picked up |
...
| 12c | **F2** · the review-money 500 (GAP-90, server half) + the close-month tone (GAP-91) | **S** | **New 9 Aug, and it goes ahead of B9 on this queue's own live-and-silently-wrong rule.** `GET /api/partner/{userId}` throws a `TypeError` — a 500 — for a **revoked** member, because the handler's 404 guard (`findBusinessMemberUserId`) does not filter `revoked_at` while `listPartnerCashPositions` does; `ReviewMoneyScreen` then renders that 500 as `Loading…` forever, having no `isError` branch at all. **Re-scoped by the tenth pass, same day.** The original plan was "fix both halves here" — but GAP-101 then showed the client half is 50 screens, not one, and belongs in a shared `QueryState` primitive under B9. So **F2 is now the server half only**: remove the `!`, decide what a revoked member's summary should return (a 404 from the guard is the obvious answer — make `findBusinessMemberUserId` filter `revoked_at` like every other reader does), and the 500 stops existing. The *spinner* stays until B9's primitive lands, on this screen and forty-nine others, which is the honest sequencing rather than fixing one screen twice. **GAP-91 rides along** — `CloseMonthScreen` confirms the one irreversible action in the product with the ordinary blue primary button; `DialogConfirmFooter` already takes `variant="destructive"` and `VoidExpenseSheet` already passes it for a lesser action. One prop, one test |
| 13 | **B9** · the remaining UI fixes | M–L | **All seven re-confirmed against source 8 Aug, none stale.** GAP-44 (`VEHICLE_DOUBLE_BOOKED` still has **zero** references anywhere in `web/src` — INV-1's blocking dialog does not exist, and §9.3's "fix offered inline" copy doesn't either) · GAP-45 (trip title still concatenates two `year: "numeric"` dates) · GAP-46 (the occupied calendar cell is still a bare `<div>` with the glyph `aria-hidden`) · GAP-47 (M-26 landscape: `grep -rn landscape web/src` still returns nothing — the largest of the seven, it touches two shared primitives) · GAP-48 (`toast-root` is still an empty div) · **GAP-55, widened** (there is **no `autoComplete` attribute anywhere in the client**, not just on the driver's Name field) · GAP-83 (`DateField`'s `sr-only` native input is still in the tab order). ~~GAP-85~~ closed 9 Aug, out of band. **GAP-101 is the biggest thing in this item and is new 9 Aug**: the client has no read-error contract at all — `isError` is checked on a *query* in two files out of 52, so a failed fetch renders as `Loading…` forever on nearly every screen, including all six reports and two Review tabs this session itself shipped. It wants a shared `QueryState` primitive (pending/error/empty/success), not 50 call sites, and a small `doc-change` to add it to UI §6's inventory. **Three more added 9 Aug: GAP-89** — `tabForPathname` has four entries against a route tree with far more, so `/reports*`, `/review*`, `/period/close` and every entity detail route highlight `Home`; **it carries a product decision, not just a map** (do `/review*`/`/reports*` belong in the Review shell for an owner-manager, and what should a detail route reached from three places highlight?) · **GAP-96** (nothing shows the cost total that voiding is supposed to change, so LT-6a's own assertion cannot be made) · **GAP-97** (`Book trip` missing from the vehicle-actions sheet on a charter vehicle — the mirror of B10's own finding) |
