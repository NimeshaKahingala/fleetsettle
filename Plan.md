...
**S0-4 · GAP-124a, the shared primitives (M, the overrun risk).** `StatTile` is genuinely new — confirmed absent from `design/primitives/`. `Section` (a primitive), `EmptyState` and `NotAvailable` (both in `components/`) exist and gain variants; plus row icon/accent conventions and the desktop rail/content constraint. **Absorbs B16 Phase 2.** Two standing traps to clear rather than rediscover: **no raw hex, `--color-*` tokens only**, and **any new token must be added to `theme.text`/`theme.spacing` in `cn.ts` or tailwind-merge silently drops it**. This is the only open-ended item in Step 0; if it starts growing into route work, that is GAP-124b and it belongs in Wave 7.
