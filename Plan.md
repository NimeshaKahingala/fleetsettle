...
| | Order | Note |
|---|---|---|
| **Track A** | ~~A9a~~ → ~~A6~~ → ~~GAP-74~~ → ~~GAP-41~~ → ~~GAP-72~~ → A10 → A13 → A15 → A16 → A8 → A14 → A7 → A9b | A9a done 5 Aug, A6 done 6 Aug, **the three report-query gaps done 8 Aug** — one file (`queries/reports.ts`), GAP-72's fix caught and corrected against real Postgres behaviour along the way. A10/A13/A15/A16/A8/A14/A7 are all free next, A9b last |
...
| 12 | **B4** · Review shell + phase-1 reports | **L** | The entire product for the partner who reads rather than enters — a real role now, since A11. **Scope settled 8 Aug (0.2): the phase-1 six, not nine** — UC-73/77/78/79 are phase 2 in FL §9.2's own per-row column. Two waves: Wave 1 is the shell, four tabs, the catalogue and the six reports (two of them honestly short); Wave 2 follows GAP-70/71, both still open — **GAP-41 and GAP-74 are done (8 Aug) and no longer block anything here**. Design and full reasoning in `B4-REPORTS-DESIGN.md`. Still the largest Track B item, but smaller than the XL it was scoped at |
...
| 15 | **B9** · `UI-UX-REVIEW.md` fixes | M | GAP-44–48: no blocking `Dialog` for INV-1, trip-title truncation, calendar cells fail a screen reader, M-26 landscape wholly unbuilt, M-11's undo toast has no host. Plus **GAP-55** (7 Aug, live): the "Add a driver" form's Name field has no `autocomplete` attribute, and **GAP-83** (8 Aug, the QA rerun): every `DateField` exposes a 1×1 focusable native input in the tab order. **Four closed 8 Aug**, re-confirmed live and fixed the same session: **GAP-76** — `driverFormSchema`/`vehicleFormSchema` gained field-specific `.min()` messages ("Name is required", "Registration is required", "Vehicle type is required"), replacing zod's generic fallback · **GAP-78** — `TwoBalances`' `driverName` is now optional, and `DriverDetailScreen` stops passing it, since `Screen`'s own title already names the driver (checked `VehicleOverviewScreen`'s superficially similar "Registration: CAB-1234" first — that one is a labeled data row beside Type/Arrangement, not a duplicate heading, and was correctly left alone) · **GAP-79** — "No description recorded" → "Incident with no description", both call sites · **GAP-80** — the three leaked phase references (`Web-P8b`, "not built yet") replaced with product copy. **GAP-81 left this bucket 8 Aug** — promoted to 10b and done there, because a phase-1 correction flow with a live backend was not a UI nit. **Still open: GAP-44–48, GAP-55, GAP-83** |
