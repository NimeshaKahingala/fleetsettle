...
| `api/src/db/schema.ts` | **Read-only for 5A; append-only for 5B**, and only for A14's share-link table (migration `0024`, decision 4). Wave 1 landed every other column this wave writes — **a second Wave 5 migration means a fact nobody planned for; stop and say so rather than adding it** |
