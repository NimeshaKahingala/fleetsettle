...
| **GAP-135** — weekly settlement rhythm | **a refusal instead of a wrong number** | `effective_due_on` is never derived from `settlement_rhythm`, so F-4.5's "a weekly settler is not in arrears on Thursday" is unmet. The write path now returns `SETTLEMENT_RHYTHM_UNSUPPORTED` rather than recording a due date that reads as overdue. **Deliberately not constrained away** — UC-31/UC-37/F-4.5 all describe weekly settlement as real, so a `CHECK` pinning `'daily'` would have been a product decision disguised as a schema fix |
