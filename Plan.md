...
| **Track B** | ~~B0~~ → ~~B11~~ → ~~B10~~ → ~~B14~~ → ~~B0b~~ → ~~B12~~ → ~~B13~~ → ~~B3 (core)~~ → ~~GAP-81~~ → ~~B4 Wave 1~~ → ~~12a `doc-change`~~ → ~~GAP-85~~ → ~~B4 Wave 2~~ → ~~F2 (client half)~~ → **B9 → B15 → B6** → B5 → B2 → B7 | **B14, B0b, B12, B13, B3-core, GAP-81, B4 (both waves), 12a's `doc-change`, GAP-85 and F2's client half (GAP-101) all done — B4 and F2 both fully closed 10 Aug 2026.** GAP-101 also closed the largest single item that had been sitting inside B9. **B5 and B2 stay behind them, deliberately and at a cost** — see "What this re-sequence gives up". **V1 is not code**, does not block any of this, and can run in parallel whenever picked up |
