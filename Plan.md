...
| | Order | Note |
|---|---|---|
| **Track A** | ~~A9a~~ → ~~A6~~ → ~~GAP-74~~ → ~~GAP-41~~ → ~~GAP-72~~ → ~~F1~~ → ~~F2 (server half)~~ → ~~A17~~ → ~~A10a~~ → A10b → ~~A13~~ → ~~A15~~ → ~~A16~~ → A8 → A14 → A7 → A9b | A9a done 5 Aug, A6 done 6 Aug, **the three report-query gaps and F1 done 8 Aug** — F1 closed GAP-84 and found one sibling (GAP-87, unfixed, correctly unscheduled). **F2, A17, A10a, A13, A15 and A16 all done 9 Aug** — F2's server half closed the revoked-member 500 (GAP-90), A17 closed two route-def validation holes (GAP-92, GAP-93), A10a shipped the management-fee generator (GAP-39, migration `0011`), A13/A15/A16 closed GAP-54/GAP-62/GAP-77. **Only A10b is still open** — it needs a `doc-change` first. A8/A14/A7/A9b are deliberately deferred behind the client work this pass prioritises |
