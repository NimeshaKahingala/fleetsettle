...
**The gate, decided 17 Aug 2026: all four live-test items must pass first — LT-9, LT-10, LT-12, LT-13.** With GAP-65, GAP-135 and GAP-136 moved to phase 2 and GAP-18 built the same day, **there is no buildable phase-1 gap left at all, so the release is gated entirely on evidence**. **LT-12 and LT-13 both closed the same day**, live against `qa.fleetsettle.com` — LT-12's `Confirm and go live` re-confirmed twice on the real QA batch (both 200, idempotency directly witnessed, one honest caveat that the batch's customer half had already self-healed before this session); LT-13's sweep narrowed the live 31-item list to the six real sheet-closes-and-navigates call sites in the whole client, five confirmed live, one (`LeaseHubScreen`'s "Close the lease") deferred for lack of a QA vehicle to test against and because closing a lease is one-way. Full accounts in TRACKER.md's own 17 Aug §0 entry and LIVE-TEST-PLAN.md. **What remains is LT-9 and LT-10 — the two items this session's own tooling cannot supply.** **LT-10** is the one to be least willing to skip: GAP-134 replaced the sheet-history mechanism with `CloseWatcher`, which by deliberate decision has **no iOS Safari fallback**, and it has only ever been verified under CDP emulation, LT-13's included — nobody has opened this build on a real iPhone. **LT-9** needs a linked-driver account, which needs a real identity to redeem the invite.
