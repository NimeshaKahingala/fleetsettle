...
**Updated 10 August 2026, later still — B9's two decisions made, and `feature/image-upload` found further along than this file's own A7 section knew.** GAP-44 and GAP-89 were the two rows in B9 carrying a product decision rather than a build question; both asked directly, both answered (GAP-44 against the recommended cheaper option, GAP-89 as recommended both times) — full wording in their own TRACKER.md rows, since the *reasoning* lives there and this file only tracks sequencing. **Practical effect: B9 is now fully unblocked**, though GAP-44 is bigger than its row previously sized it, since the chosen answer touches the backend error class and its wire schema, not only the client. **Separately, `feature/image-upload` turned out to be a complete seven-commit build**, not "fully planned, not started" — see A7's own section below, corrected to match. **This does not change the order above**: F3 still leads, because GAP-103 is unbuilt *and* still carries its own undecided fork (materialise the six opening-balance kinds synchronously, or keep them a separate ledger the reports read alongside the real tables) — nobody has been asked yet, and the user's own next session is going to `feature/image-upload` to close out A7's verification gap rather than to F3, so the documented order and the actual next session diverge here by choice, not by mistake.

**Updated 10 August 2026, later still — A7 merged (PR #20, into `develop`, not yet `main`); `PENDING-REVIEW-ITEMS-2026-08-10.md` evaluated (TRACKER §6's twelfth pass) and nine of B9's rows built the same sitting.** A7/GAP-16 is closed for expense receipts, so Track A's remaining queue is A8, A14, A9b — full account in TRACKER §0. GAP-45, GAP-46, GAP-55, GAP-89, GAP-91, GAP-96, GAP-97 and GAP-99 all closed (plus M-25, riding with GAP-97); TRACKER §4's closed table has each. **B9 is not fully closed** — GAP-44, GAP-47, GAP-48, GAP-83, GAP-105 and GAP-106 remain, none of them a same-pass extension of the nine just built (GAP-44 needs the wire-schema change its own decision requires; GAP-47/48/83 each touch a shared primitive or need a new one). **F3 and F4 are untouched and still lead the documented order** — this pass built the smallest, least-ambiguous items ready regardless of sequence, not a re-ordering past GAP-103's own undecided fork.
