...
**Current order: B13 remainder/GAP-100 → B2 remainder → A14 → A8 → A9b → B7**, with V1 (live testing) running in parallel whenever picked up. **F4, F3, the low-API B9 primitives, B15, B6, B5's core screens, and the safe B2 callers all closed 11 Aug** (GAP-104, GAP-103, GAP-108, GAP-47, GAP-48, GAP-82, GAP-22, GAP-102, GAP-67 plus the paid-by picker — see their rows and TRACKER.md's closed table). GAP-44 is the only B9 row left, but it is wire-schema work, not part of the low-API UI batch. B2's remaining setup surfaces are ownership-share and management-agreement UI plus GAP-1's access scoping; they should not be smuggled into a cash screen that implies per-vehicle access control now exists.
...
**11 August 2026 — the low-API UI branch closed B6, B5's core, and the safe B2 callers.** B6 replaced `/people/customers/:id` with a real customer detail over A4's reads and added the small vehicle-paperwork renewal sheet that closes GAP-102. B5 now has the driver's own `/me` screen over `GET /api/driver-view`, a shared read-only driver activity section, and the staff-side `DriverDetailScreen` history read over `GET /api/driver/{id}/view`; A14's signed printed-slip link is still separate backend work, so the statement-link line remains there. B2 now has `/cash`, `/partners/:userId`, partner capital contribution, partner payout/settlement, banking events, the owner-facing Members screen, the driver access sheet on driver detail, GAP-67's mileage-package create/archive screen, and GAP-31's paid-by picker in `RecordExpenseSheet`. **Not closed by this branch:** ownership-share setup UI, management-agreement/share-vehicle UI, and GAP-1's per-vehicle access scoping. Those are deliberately left as B2 remainder rather than shipping UI that suggests a manager invite is safe for a real business before GAP-1 lands.
...
| **Track B** | ~~B0~~ → ~~B11~~ → ~~B10~~ → ~~B14~~ → ~~B0b~~ → ~~B12~~ → ~~B13~~ → ~~B3 (core)~~ → ~~GAP-81~~ → ~~B4 Wave 1~~ → ~~12a `doc-change`~~ → ~~GAP-85~~ → ~~B4 Wave 2~~ → ~~F2 (client half)~~ → ~~F4~~ → ~~F3~~ → ~~B9 low-API primitives~~ → ~~B15~~ → ~~B6~~ → ~~B5 core~~ → **B2 remainder** → B7 | **B14, B0b, B12, B13, B3-core, GAP-81, B4 (both waves), 12a's `doc-change`, GAP-85, F2's client half (GAP-101), F4 (GAP-104), F3 (GAP-103), B9's low-API primitive remainder (GAP-108, GAP-47, GAP-48), B15 (GAP-82), B6 (GAP-22/GAP-102), and B5's core screens are done.** GAP-44 remains from B9 but needs the wire-schema change its own decision requires, so it is not part of this low-API UI slice. **B2 remainder is now the frontend head:** ownership shares, management agreements/share-vehicle, and any GAP-1-safe access-scoping presentation. **V1 is not code**, does not block any of this, and can run in parallel whenever picked up |
...
| 21 | **B2 remainder** · ownership/share setup and scoping-safe partner admin | M | `/cash`, `/partners/:userId`, capital contribution, banking, partner payout, mileage packages and paid-by picker all built 11 Aug. Remaining: ownership shares, management agreements/share-vehicle, and GAP-1's per-vehicle access scoping/presentation |
