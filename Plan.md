...
**S0-3 · Close the guard gap (S–M, and the one step that can surprise).** [`checkQueryErrorHandling`](scripts/check-forbidden.mjs#L297) skips an entire file when `useQueryState` or `QueryState` appears anywhere in it, which is exactly how three error-only wirings exempted every unhandled pending state beside them. **Run the tightened rule across the whole repo before settling its shape** — Wave 2 learned this on the sibling `money/void-table-unfiltered` rule, where a first draft would have failed the gate on nineteen pre-existing gaps in files that session never touched. Expect other pending-state misses; **triage and file them, do not fix them all here** — Step 0 is a prelude, not a client sweep.
