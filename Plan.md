...
>
> **What is left, in one line — and since GAP-18 closed 17 Aug 2026 it is entirely evidence, not code:** **zero** buildable gaps — Wave 8's four live-test items (LT-9, LT-10, LT-12, LT-13), then Wave 9 · Release. **GAP-65, GAP-135 and GAP-136 all moved to phase 2** by owner decision; GAP-131/GAP-132 and GAP-18 (both halves — Wave 6) all closed.
>
> **LT-13 is the one to run first.** It is the phase-1 mobile interaction sweep, it has found a real defect on every pass — GAP-125, then GAP-134 — and it is nowhere near finished. **LT-10 is the one to be least willing to skip:** `CloseWatcher` has no iOS Safari fallback by design and has only ever been tested under emulation.
>
...
**The gate, decided 17 Aug 2026: all four live-test items must pass first — LT-9, LT-10, LT-12, LT-13.** With GAP-65, GAP-135 and GAP-136 moved to phase 2 and GAP-18 built the same day, **there is no buildable phase-1 gap left at all, so the release is gated entirely on evidence**, and the two items carrying that weight are worth naming individually. **LT-10** is the one to be least willing to skip: GAP-134 replaced the sheet-history mechanism with `CloseWatcher`, which by deliberate decision has **no iOS Safari fallback**, and it has only ever been verified under CDP emulation — nobody has opened this build on a real iPhone since. **LT-13** is the one most likely to return something: it has found a real defect on every pass ever run (GAP-125, then GAP-134), and its own row still reads "the rest of the sweep remains open."
