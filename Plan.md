...
- **B4 sequences in two waves, cut by contract completeness rather than by surface.** The review proposed a Review-core / catalogue split; that puts the blocked work first, because the Review shell's headline tab was the part gated on GAP-41 while two catalogue reports were unblocked today. Wave 1 ships everything that is true now; Wave 2 followed the Track A increments — **GAP-41 and GAP-74 are both done (8 Aug), so Wave 2 is down to GAP-70 and GAP-71**, both still waiting on their own `doc-change`.

**Updated 8 August 2026 — B14, B0b, B12, B13 and B3's core all shipped in one session.** Six commits (`853c916` B14, `cd14d5d` B0b, `6736e62` B13, `12eefa3` B12, `5ddadad` B3, `d0e3e82` the router/`/more` wiring), full accounts in each item's own section below. One thing worth restating because it's easy to lose in six items landing together: **B13 needed a real backend fix, not just a screen.** `POST /api/payment` had only ever accepted `direction: "received"` — "pay the driver" had no backend path to a driver's `owed_by_us` balance at all, not merely no caller, the same one-layer-deeper shape GAP-51/54 already established for a missing flow. **B3 shipped its core only** — `WriteOffSheet`/`PostClosureChargeSheet` need a specific obligation/lease/trip as an entry point the close screen never had; recorded as item **10a** in the queue below rather than a checked box that wasn't true. **Superseded within hours by the triage pass below** — "next item is B4" turned out to be wrong twice over: 10a was withdrawn as phase 2, and four smaller items (GAP-81, then the Track A trio) outranked B4 on the live-wrong-money and hard-prerequisite rules this queue already runs on.

**Updated 8 August 2026 — a QA rerun re-confirmed most of the 8 Aug findings and surfaced one new one, then GAP-81 and the Track A trio (GAP-72/41/74) all shipped in the same session, clearing everything ahead of B4.** `QA-BROWSER-TEST-FINDINGS-2026-08-08-RERUN.md`, validated the same way every external review here is: source-checked, not absorbed on trust. Confirmed live: B14's fix (a booked trip now shows its real due figure), and re-opened GAP-76/78/79/80/81 exactly as filed. One claim didn't hold — "Close the month can be opened while status shows all zeros" describes U-7 working as designed (warns and lists, never blocks), not a defect; recorded in TRACKER §6 rather than scheduled. **One genuinely new finding, GAP-83**: every `DateField` — not only the one screen it was reported on — exposes a 1×1 focusable native date input in the tab order, `sr-only` but still reachable by keyboard with no visible focus indicator. Filed under B9.

**GAP-81 done**: one shared `ExpenseCostRow` replaces three screens' identical inline cost-row block; a live row opens `VoidExpenseSheet` (reason required), a voided one stays struck through and un-tappable. Surfaced two more stale duplicate `EXPENSE_CATEGORY_LABEL` maps along the way, both deleted. **The B9 copy batch closed alongside it** — GAP-76 (field-specific validation messages), GAP-78 (`TwoBalances`' `driverName` made optional so `DriverDetailScreen` stops repeating its own `Screen` title — checked `VehicleOverviewScreen`'s similar-looking "Registration" field first and confirmed it's a different, legitimate shape, not the same bug twice), GAP-79 ("Incident with no description"), GAP-80 (three leaked phase references replaced with product copy).

**Then the Track A trio, GAP-72/41/74, all in `queries/reports.ts` — one visit instead of three.** GAP-72 is the one worth reading: **the fix written down when this gap was found was itself wrong**, confirmed by running the raw SQL directly against Postgres rather than trusting the plan — `date AT TIME ZONE tz` silently resolves to a different overload than `timestamp AT TIME ZONE tz` and lands on a value eleven hours off. Caught only because this queue's "make it fail on purpose" rule was applied *twice*: the new tests failed pre-fix as expected, and then one **kept failing against the first-draft fix**, which is what exposed the wrong overload rather than a passing test hiding it. GAP-41 and GAP-74 both landed clean on the first attempt — full accounts in each gap's own TRACKER row and this file's Track A section below.

**Updated 8 August 2026 — the queue is re-sequenced around finishing B4, and this is the current order.** Everything above stays as the record of how it got here; **["The order, end to end"](#the-order-end-to-end) is what to work from**, and ["The 8 August re-sequence"](#the-8-august-re-sequence--what-it-prioritises-and-what-it-gives-up) is why it reads the way it does. In short: **merge → F1 → B4 (Wave 1 → `doc-change` → Wave 2) → B9 → B15 → B6 → A10a → A10b → A13 → A15 → A16**, with B5, B2, A14, A8, A7, A9b and B7 behind them.
...
| | Order | Note |
|---|---|---|
| **Track A** | ~~A9a~~ → ~~A6~~ → ~~GAP-74~~ → ~~GAP-41~~ → ~~GAP-72~~ → ~~F1~~ → **A10a → A10b → A13 → A15 → A16** → A8 → A14 → A7 → A9b | A9a done 5 Aug, A6 done 6 Aug, **the three report-query gaps and F1 done 8 Aug** — F1 closed GAP-84 and found one sibling (GAP-87, unfixed, correctly unscheduled). **A10 splits in two** — A10a builds, A10b needs a `doc-change` first. A8/A14/A7/A9b are deliberately deferred behind the client work this pass prioritises |
| **Track B** | ~~B0~~ → ~~B11~~ → ~~B10~~ → ~~B14~~ → ~~B0b~~ → ~~B12~~ → ~~B13~~ → ~~B3 (core)~~ → ~~GAP-81~~ → ~~B4 Wave 1~~ → **`doc-change` → B4 Wave 2 → B9 → B15 → B6** → B5 → B2 → B7 | **B14, B0b, B12, B13, B3-core, GAP-81 and B4 Wave 1 all done.** **B4's own `doc-change` and Wave 2 are still open** — stopped there deliberately, not blocked, so the next session either finishes B4 (12a → 12b) or moves straight to B9, both legitimate next steps. **B5 and B2 stay behind them, deliberately and at a cost** — see "What this re-sequence gives up". **V1 is not code**, does not block any of this, and can run in parallel whenever picked up |
...
| 11d | ~~**F1** · the arrangement is validated on neither side (GAP-84)~~ | **S** | ✅ **Done 8 Aug 2026.** `VehicleArrangementMismatchError` (409) in both `startLeaseHandler` (requires exactly `"A"`, `null` refused too) and `startDailyLeaseHandler` (requires `"B"` or `null`, matching the client's own `canStartDailyLease`); both screens gained a guard rendering a plain message with a back action instead of the form. **Found in passing, not fixed**: `bookTrip` has the identical unvalidated shape — now **GAP-87**, correctly unscheduled pending a `use-cases.md` read on whether an opportunistic charter should be arrangement-restricted at all. Full account in TRACKER.md |
...
| 12a | **`doc-change`** · DM §15 (GAP-70) + UI §11.1 (GAP-71) | S | **Still open — stopped here deliberately, on explicit instruction, not blocked.** Both are cases where the requirement went missing from the owning document, not from the code: DM §15's cash-position query is headed "held per partner" and never carried down UC-75's "and account", and UI §11.1's UC-76 row dropped the reason breakdown FL §9.2 marks phase 1. **The documents move first, then the queries** — changing the query alone would leave both screens unspecified, and this repository does not smuggle a design decision in as a side effect of a build |
| 12b | **B4 Wave 2** · what the doc-change unblocks | S–M | **Still open.** UC-75 gains `banked[]` + `driverAdvances[]` and the title reverts to "Where is our cash"; UC-76 gains its real reason/month contract once 12a settles what it should be. **This is what finishes B4** — Wave 1 ships one report that is honest but short (cash-position) and one that ships a different shape than the wireframe named (lost-days), and leaving either there indefinitely is how "narrow but true" becomes permanent. Both figures already exist in the schema (`banking_event.destination` is `NOT NULL`; `day_record.lost_reason` is `CHECK`-constrained and non-null whenever the day is lost), so neither needs a migration |
| 13 | **B9** · the remaining UI fixes | M | **All seven re-confirmed against source 8 Aug, none stale.** GAP-44 (`VEHICLE_DOUBLE_BOOKED` still has **zero** references anywhere in `web/src` — INV-1's blocking dialog does not exist, and §9.3's "fix offered inline" copy doesn't either) · GAP-45 (trip title still concatenates two `year: "numeric"` dates) · GAP-46 (the occupied calendar cell is still a bare `<div>` with the glyph `aria-hidden`) · GAP-47 (M-26 landscape: `grep -rn landscape web/src` still returns nothing — the largest of the seven, it touches two shared primitives) · GAP-48 (`toast-root` is still an empty div) · **GAP-55, widened** (there is **no `autoComplete` attribute anywhere in the client**, not just on the driver's Name field) · GAP-83 (`DateField`'s `sr-only` native input is still in the tab order) · **plus GAP-85**, new — `MoneyField` renders `Rs 0` for an empty value |
...
| 20 | **B5** · Mine shell | M | The entire product for the linked driver — a real role now, since A11, landing in a real (empty) shell since B0b. `GET /api/driver-view` has been ready since P12. **Moved here 8 Aug by the re-sequence, and it is the one item this pass genuinely gives up something to defer** — see below |
...
| 25 | **A9b** · the rest of soft delete | L | ~15 near-identical void endpoints (GAP-12, GAP-36) plus GAP-60's `replaces_id` linkage, which should be decided **before** fifteen void paths are written without it. A batch to grind, not a design problem |
...
**Two things deliberately *not* moved.** GAP-1 stays unscheduled (it needs a design decision — which table decides "his" vehicles — before it needs a slot, and the operational guard stands: **do not invite a `manager` to a real business**). GAP-58, the 178-case test manifest that has never been run, also stays out: it is the systematic version of what V1 does by hand, and adding it to a queue this long would be scheduling it in name only.
...
| **A13** | **Change a vehicle's arrangement (F-1.2)** — **new 7 Aug** | GAP-54 | 1 | F1's refusal (an honest wall needs a way through it) |
...
| **A10a** | The management fee that never fires | GAP-39 | 0 + a generator | — |
| **A10b** | The incident contribution that is not a receivable — **needs a `doc-change` first** | GAP-10 | 0–1, after F-3.4 decides | a `doc-change` on `user-flows.md` F-3.4 |
...
**Done means** — a `POST /api/lease` naming an arrangement-C vehicle 409s with a code the client can read, the two screens refuse before rendering, and both are asserted in the direction that fails today. **Pair it with A13 (row 17)**: a refusal is only honest once there is a way to change a vehicle's arrangement, and until A13 lands the message should say what to do rather than only what went wrong.
...
### A10a / A10b · The other two silent zeros — closes GAP-39, and puts GAP-10 behind a `doc-change`
...
**Done means** — **A10a:** a managed vehicle's profit drops by its management fee in UC-70, and `GET /api/partner/{userId}`'s directly-read `monthly_amount_minor` still agrees with what the generator writes. **A10b:** F-3.4 says which way it goes, and *then* an agreed customer contribution either shows up as a receivable a payment can settle or is recorded as deliberately not one. **Both:** G-2 still lands on 15,000.
...
**Backend increments: five, and three are done.** §7.8's overheads block had none (**GAP-41**, ✅ done 8 Aug) and the verification pass found four more — **GAP-70** (cash position loses banked money, open), **GAP-71** (lost-day reasons, phase 1, and UI §11.1 dropped it too, open), **GAP-72** (goodwill's window math, ✅ done 8 Aug) with **GAP-73** (goodwill's date basis and missing breakdown — distinct, still open) alongside it, and **GAP-74** (✅ done 8 Aug).
