...
**S0-2 · `ReasonPicker` gains a pending state (S).** Smaller than the row implies, because the machinery already exists and is unused: [`QueryState`](web/src/components/QueryState.tsx#L91) already resolves all four kinds and already takes `pendingFallback`/`idleFallback`. `ReasonPicker` takes an `error`-only prop instead. Replace that with the full query state and wire the three `QuickAddSheet` call sites; the three static-list callers (`RecordExpenseSheet`, `ConfirmDayCard`, `LostDaysReportScreen`) pass nothing and must not change — checked, they are literal arrays. **`idle` stays distinct from `pending`**, for the reason GAP-101 already recorded: these queries are `enabled`-gated, so they sit in `isPending: true, fetchStatus: "idle"` until the picker opens, and collapsing the two would render a spinner on a closed sheet forever.
