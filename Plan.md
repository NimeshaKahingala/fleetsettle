...
>
> **What is left, in one line — and after 17 Aug 2026's decisions it is mostly evidence, not code:** **one** buildable gap (GAP-18 — UC-73's yearly view and UC-99's CSV half), Wave 8's four live-test items (LT-9, LT-10, LT-12, LT-13), [Wave 8b · document dispositions](#wave-8b--document-dispositions-s--added-16-august-2026), then Wave 9 · Release. **GAP-65, GAP-135 and the new GAP-136 all moved to phase 2** by owner decision; GAP-131/GAP-132 closed 16 Aug.
>
...
**GAP-18 narrowed 17 Aug 2026, and it is now the only buildable phase-1 gap left in the plan.** The export half had been sitting behind an undecided architecture question; the decision was taken and **split the item rather than answering it**: UC-99's **CSV half stays here and is startable**, and the **PDF half becomes [GAP-136](TRACKER.md) and defers to phase 2** — no PDF renderer has a home in this runtime (`TS §8`), and the two available answers were a rendering dependency inside the runtime that performs every money write, or an external service that would receive every figure on a statement. Neither is needed for what UC-99 is for: CSV gets the numbers out, and a statement prints from the screen that already shows it. **So what remains under GAP-18 is two ordinary jobs with nothing outstanding** — UC-73's yearly view (F-9.2 already specifies it fully: "As UC-70, with overheads beneath vehicle profit, never spread across it", the same shape `getVehicleMonthReport` reads, widened to a year) and the CSV export with its W-49 permission checks. Docs moved with the decision: `use-cases.md` UC-99 v1.2.13, `user-flows.md` F-9.3 v1.1.14.
...
### Wave 8b · Document dispositions (S) — added 16 August 2026
...
| File | What is wrong | Disposition |
|---|---|---|
| **`GAP-12-VOID-CASCADE-DESIGN.md`** | Header reads *"design finalised and decided. **No code written against it yet**, and no specification document updated yet."* **The code was written** — GAP-12 closed 14 Aug 2026 as 5B-1; `api/src/route-defs/` carries 16 `/void` references and `void-cascade.test.ts` exists. Anyone reading this file learns the cascade is unbuilt | Mark built, point at GAP-12's closed row, keep the design reasoning |
| **`UI-LOW-API-WORK-PLAN-2026-08-11.md`** | A handoff plan for a Codex chat, dated 11 Aug; the branch it planned (`codex/ui-low-api-work-2026-08-11`) merged. **No status marker and referenced by nothing** — `grep` across TRACKER.md, Plan.md and CLAUDE.md returns zero | Mark superseded, or delete (recoverable from git) |
| **`AGENTS.md`** | The Codex entrypoint, and **referenced by nothing** — zero hits in TRACKER.md, Plan.md and CLAUDE.md. It restates read order and project rules, so it is a second copy of guidance CLAUDE.md owns, with nothing keeping the two honest | Make it point at CLAUDE.md rather than restate it — `.claude/rules/docs.md`'s own "two copies of a rule become two different rules" applies to tool entrypoints too |
| **`docs/testing/`** | 35 files, ~4,900 lines, **entirely unmapped by `docs/README.md`** — the index CLAUDE.md tells you to read first, which announces "Seven documents" and names only `engineering/fixtures/` and `design/brand/` as supporting material. Mitigated by the tree's own README, which is honest that it is retired (GAP-58, 11 Aug) and points at LIVE-TEST-PLAN.md instead | One line in `docs/README.md`'s supporting-material paragraph. **A specification edit — needs the `doc-change` skill, a version bump and a Status-table update**, which is why it is listed here rather than done in passing |
| **`README.md`** | Three lines, and the repository's public face. Says nothing about what FleetSettle is, how to run it, or where the documentation starts | Point it at `docs/README.md` and `CLAUDE.md`; it does not need to be long |
