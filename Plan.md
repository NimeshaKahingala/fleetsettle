...
| 9c | **V1** · one live verification pass on QA | **XS–S** | **New, 8 Aug — not code, and first in line because it is now unblocked for the first time.** PR #11 put B0b, B12, B13 and B3-core on `qa.fleetsettle.com` at 15:03. Two debts clear together in one session: **GAP-3's live confirmation**, open since 6 Aug (start a daily lease through B10's flow → let `generate-day-cards` run → confirm the day, which is the only proof the fix works against the cron that broke it), and **B0b's Review and Mine shells**, which sit exactly on the `FirstRunGate` non-operate boundary TRACKER §5's standing rule says gets one real-browser check in both colour schemes before being called done. That rule exists because mocked review shipped GAP-49 and GAP-50 on this same code. **Every live pass so far has found something no source read did** — GAP-49/50/51, GAP-54, GAP-75 — so budget for findings, not just confirmation |
