...
**Current order: B9 → B15 → B6**, with B5, B2, A14, A8, A9b and B7 behind them, and V1 (live testing) running in parallel whenever picked up. **F4 and F3 both closed 11 Aug** (GAP-104, GAP-103 — see their rows and TRACKER.md's closed table). GAP-103's own fork — materialise on commit vs. a separate ledger — was asked and came back **materialise**; built the same day.
...
| **Track B** | ~~B0~~ → ~~B11~~ → ~~B10~~ → ~~B14~~ → ~~B0b~~ → ~~B12~~ → ~~B13~~ → ~~B3 (core)~~ → ~~GAP-81~~ → ~~B4 Wave 1~~ → ~~12a `doc-change`~~ → ~~GAP-85~~ → ~~B4 Wave 2~~ → ~~F2 (client half)~~ → ~~F4~~ → ~~F3~~ → **B9 → B15 → B6** → B5 → B2 → B7 | **B14, B0b, B12, B13, B3-core, GAP-81, B4 (both waves), 12a's `doc-change`, GAP-85, F2's client half (GAP-101), F4 (GAP-104) and F3 (GAP-103) all done — B4 and F2 both fully closed 10 Aug 2026, F4 and F3 both closed 11 Aug.** GAP-101 also closed the largest single item that had been sitting inside B9. **B9 is now the head of the queue** — GAP-44/47/48/83/105, its own remainder. **B5 and B2 stay behind it, deliberately and at a cost** — see "What this re-sequence gives up". **V1 is not code**, does not block any of this, and can run in parallel whenever picked up |
...
| 13 | **B9** · the remaining UI fixes | M | **GAP-44 and GAP-89 decided 10 Aug** (TRACKER.md carries the reasoning). **Eleven of fourteen items closed 10–11 Aug** (TRACKER §6's twelfth pass, plus GAP-83/GAP-105 the day after): GAP-45, GAP-46, GAP-55, GAP-89, GAP-91, GAP-96, GAP-97, GAP-99, GAP-106, GAP-83, GAP-105 (plus M-25, riding with GAP-97). **What's left: GAP-44, GAP-47, GAP-48** — GAP-44 needs the wire-schema change its own decision requires; GAP-47/48 each touch a shared primitive (`AppShell`/`Screen`'s landscape variant; a new `Toast`) |
