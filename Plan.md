...
| 12d | **F3** · opening balances confirm live, connect to nothing (GAP-103) | M | **Still open — the most severe gap this project has recorded, and still unbuilt.** `commitOpeningBalance` writes and status-flips `opening_balance_batch`/`opening_balance_entry` only; nothing turns a committed entry into a real `obligation`/`deposit`/`advance` row, so a confirmed starting position is silently invisible to every report and balance. **Needs a decision before any code moves**: materialise the six entry kinds synchronously on commit, or keep them a separate ledger the reports learn to read alongside the real tables — nobody has been asked yet. Full account: `LIVE-BROWSER-FINDINGS-2026-08-10.md` F-0, TRACKER.md GAP-103 |
| 12e | **F4** · mobile `ActionSheet`/`Sheet` history race (GAP-104) | M | **Still open, live-and-silently-wrong on any real touch device.** Blocks Quick Add's Fuel/Expense/New trip on any real phone. Fix is proposed in full in `MOBILE-SHEET-AND-DATE-PICKER-FINDINGS-2026-08-09.md` — a sheet-stack/history-ownership manager in `ActionSheet.tsx`/`useMobileHistoryDismiss.ts`. Full account: `LIVE-BROWSER-FINDINGS-2026-08-10.md` F-1/F-2, TRACKER.md GAP-104 |
| 13 | **B9** · the remaining UI fixes | M | **GAP-44 and GAP-89 decided 10 Aug** (TRACKER.md carries the reasoning). **Nine of fourteen items closed 10–11 Aug** (TRACKER §6's twelfth pass): GAP-45, GAP-46, GAP-55, GAP-89, GAP-91, GAP-96, GAP-97, GAP-99, GAP-106 (plus M-25, riding with GAP-97). **What's left: GAP-44, GAP-47, GAP-48, GAP-83, GAP-105** — none a same-pass extension of what closed (GAP-44 needs the wire-schema change its own decision requires; GAP-47/48/83 each touch a shared primitive or need a new one) |
| 14 | **B15** · quick-add's two payment actions (GAP-82) | S–M | Smaller than it reads — `recordPaymentRequestSchema` already carries `direction`/`partyType`; only a party picker is missing |
| 15 | **B6** · customer detail (+ vehicle paperwork write, GAP-102) | S | Party-scoped twin of the lease hub; route already wired, only the screen is missing. Closes GAP-22 |
...
| 18a | **B13 remainder** · settle an advance (GAP-100) | S | `POST /api/advance/{id}/settle` exists; nothing in `web/src` calls it, stranding INV-17's own close-blocking flow. Reuses `AdvanceSheet`'s shape |
...
| 20 | **B5** · Mine shell (+ the staff driver-history read) | M | **Two screens, not one** — the driver's own Mine shell over `GET /api/driver-view`, and `DriverDetailScreen`'s still-uncalled staff history read (ACA-06). The one item this queue's 8 Aug re-sequence deliberately deferred at a real cost — see below |
| 21 | **B2** · partners, banking, cash | M | Six screens backed by A2, plus mileage-package create (GAP-67) and partner-payout (GAP-69). Needs no B0b |
