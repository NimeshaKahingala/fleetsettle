...
| **GAP-13** | The close checklist does not count "unconfirmed days". **The reason expired when P13 shipped:** `generate-day-cards` only generates cards for pattern days, so a `day_record` in `state = 'open'` *is* a scheduled-but-unconfirmed day — one `COUNT(*)` scoped to the open period, not a pattern replay. UI §7.7 makes it the checklist's **first row**, so B3 cannot render its own spec without it. Under-reports if the cron hasn't run, which is honest and warn-only (U-7) | **A3** |
...
| **GAP-38** | **`GET /api/payment` does not exist, so `POST /api/payment/{id}/correct` is unreachable from any interface.** The correction endpoint has been built and tested since P9, but `queries/payment.ts` has no list function and no route exposes one — nothing can hand a screen a payment id. F-8.2's first step is literally "open the receipt", so B3's whole correction half is blocked on it. Carries the linked-driver test class; payments name drivers | **A3** |
