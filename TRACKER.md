...
**Not built this pass, recorded rather than guessed at**: `dispatch-messages` (P14's own prerequisite, nothing to dispatch into yet); the `paperwork-warnings`/`deposit-hold-release` **reads** themselves (a home-screen query against `vehicle_document.expiry_date` / `deposit.hold_release_date` — real, small, buildable work, just not reached this pass); the WhatsApp-reminder half of either, which needs P14 regardless.

**Depends on** — the flows each job mirrors, which is why it sits here rather than earlier.

**Verification** — 8 new tests: 6 in `tests/integration/scheduled.test.ts` (every_day/weekdays/alternate pattern generation, the open-period clip, an existing allocation respected, idempotency, and billing-period catch-up across 3 periods), 2 in `packages/shared/src/dates.test.ts` for the new `weekdayOf` helper — run via `test:integration` against the live Neon test DB. Both scheduled-job domain functions are called directly rather than through HTTP (IG §3.1: `domain/` takes `(db, …)`, no request in sight), and every assertion reads back rows scoped to the test's own vehicle/lease/daily-lease id rather than the functions' own aggregate counts — those counts are correct but also include whatever else is active elsewhere in this shared, long-lived Neon test branch at the moment the test runs (a fresh per-run branch, IG §9.2's intended CI setup, would not have this problem; this interactive session's one persistent branch does). `npm run check` clean across all three workspaces; the full `test:integration` suite (26 files, 254 tests) passes with no regressions.
